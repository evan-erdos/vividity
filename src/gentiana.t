...
"Gentiana is a bookish kind of girl, with round glasses and very curly, auburn hair. She is slightly older than you, and is Ada\'s best friend. The two of you get along just fine, but you only know her through Ada, and you\'re not sure if you have anything in common. " {
	...
	Harm (n) { // n being damage, positive or negative
		gentiana_teuta.health-=n;
		if (gentiana_teuta.health<0) gentiana_teuta.Kill();
	}

	Kill () {
		"\bYou hit her square in the head, and she crumples to the ground with an exasperated whimper. ";
		gentiana_teuta.isDead = true;
		gentiana_teuta.setCurState(gentiana_dead);
	}

	uselessToAttackMsg {
		if (gentiana_teuta.isDead) return 'You\'ve done enough.';
		combat.attack(gentiana_teuta);
		user.commitCrime(8);
		return (gentiana_teuta.isDead)?' ':noKillMessages.getNextValue;
	}

	noKillMessages : ShuffledList {
		valueList = ['Her eyes go wide as she looks at you, aghast. \"What... what are you doing?\" She looks terrified.','She dodges your attack, shrieking as she stumbles to the side and runs to the other side of the room.', 'You swing and hit her shoulder, but it doesn\'t seem to have hurt her severely. She lets out a desperate, mortal gasp as you do so.'];
	}

	cannotKissActorMsg { return noKissMessages.getNextValue; }
	noKissMessages : ShuffledList {
		valueList = ['\"<<user.firstname>>, please, don\'t.\" she tugs away from you.','\"<b>Stop</b> it.\" she pushes you further away, and gives you a cold look.','\"<b>Get the fuck away from me, <<user.firstname>></b>.\" Her eyes dart back and forth between you and the door.'];
	}
}
...
+ gentiana_dead : ActorState {
	stateDesc = "It's only a matter of time before someone finds out that you've killed her. ";
	specialDesc = "Gentiana's body is here, crumpled in the corner and face down. ";
}

++ DefaultAnyTopic { "She isn't likely to respond."; }
